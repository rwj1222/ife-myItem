function plotBloc(data){
    ...
    var blockWidth=40/data.length;     //柱状图的宽度
    ...
    for (var i=0;i<5;i++){
        ...
        text.setAttribute("y",(i)*70+40);
        text.textContent=max-(max/5)*i;
    ...
    for (var i=0;i<data[j]["sale"].length;i++){
        ...
        // var text=document.createElementNS("http://www.w3.org/2000/svg", "text");
        // text.setAttribute("x",blockWidth*(i+1)+blockInternal*i);
        // text.setAttribute("y",380-height-10);
        // text.textContent=data[j]["sale"][i];
        // SVG.appendChild(text);
    }
